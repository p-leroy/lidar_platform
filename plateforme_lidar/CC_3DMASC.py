...
def compute_features(query0_params,workspace,params,training_file):
    ...
    if query0_params[1]=="SBF":
        ...
        cloudcompare.last_file(workspace+"_".join([params[i][0:-4],"WITH_FEATURES",today.date,"*.laz"]),params["PCX"][0:-4]+"_features.laz")
        for i in params.keys():                
...
def load_features(pcx_filepath,training_filepath,labels=False):
    ...
    for i in tab:
        ...
        if name in utils.convention.keys():
            listSF+=[utils.convention[name]]
        else:
            listSF+=[name]
    idxSelect=list(np.sort([names.index(i) for i in listSF]))
...
def classif_report(labels_true,labels_pred,ind_confid=[],save=False):
    labels=np.unique(labels_true)
    tab=metrics.confusion_matrix(labels_true,labels_pred,labels)
...
def FIA(tab,NBscales):
    """
    Feature Importances Analysis
    (by features and by scales)
    """
    if len(tab)%NBscales==0:
        NBfeat=int(len(tab)/NBscales)
    else:
        raise ValueError("Length of your array doesn't match exactly number of scales")

    by_feat=[]
    for i in range(0,NBfeat):
        p=np.arange(0,NBscales)+(i*NBscales)
        by_feat+=[np.sum(tab[p])]

    by_scales=[]
    for i in range(0,NBscales):
        p=[NBscales*m+i for m in range(0,NBfeat)]
...
def correlation(data,names,save=""):
    ...
    for i in range(0,len(data[0,:])):
        ...
        liste+=[temp]
    if len(save)>0:
        np.savetxt(save,liste,fmt='%.4f',delimiter=";")

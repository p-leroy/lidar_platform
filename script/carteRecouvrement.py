...
class Overlap(object):
    ...
    def Comparison(self,lineA,lineB,corePts,outName):
        query=PL.cloudcompare.open_file(self.CCoptions,[self.workspace+lineA,self.workspace+lineB,self.workspace+corePts])
        PL.cloudcompare.m3c2(query,self.workspace+self.m3c2File)
        PL.cloudcompare.last_file(self.workspace+corePts[0:-4]+"_20*.laz",outName)
        new_file=PL.cloudcompare.last_file(self.workspace+lineA[0:-4]+"_20*.laz")
        os.remove(new_file)
        new_file=PL.cloudcompare.last_file(self.workspace+lineA[0:-4]+"_M3C2_20*.laz")
        os.remove(new_file)
        new_file=PL.cloudcompare.last_file(self.workspace+lineB[0:-4]+"_20*.laz")
        os.remove(new_file)

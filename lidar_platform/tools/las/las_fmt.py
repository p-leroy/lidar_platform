...
def unpack_vlr_waveform_packet_descriptor(vlr, as_list=False):
    # 26 bytes
    ...
    try:
        ...
        return {
            "bits_per_sample": vlr.parsed_record.bits_per_sample,
...
def pack_vlr_waveform_packet_descriptor(descriptor):
    return pack(
        "=2B2L2d",
...
def unpack_evlr_header_waveform_data_packet(evlr, asList=False):
    # 60 bytes
...
def pack_evlr_header_waveform_data_packet(waveform_data_packet):
    # 60 bytes
    # 2 bytes, Reserved [unsigned short]
    # 16 bytes, User ID char[16]
    # 2 bytes, Record ID [unsigned short]
...
class LASFormat(object):
    def __init__(self):
        ...
        rgb = [("red", "uint16"), ("green", "uint16"), ("blue", "uint16")]

        nir = [("nir", "uint16")]

        fwf = [
            ("wavepacket_index", "uint8"),
        ...
        self.identifier = {
            "system_identifier": system_id + "\x00" * (32 - len(system_id)),
